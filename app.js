...
app.get("/", ensureAuthenticated, function(request,response){
	MongoClient.connect(url, function(err,db){
		if(err)throw err;
		var dbObj= db.db("lists");
		
		dbObj.collection("lists").find().toArray(function(err, results){
			console.log("Site Served");

			db.close();
			response.render("index", {lists:results});
		});
...
app.post("/new-entry", function(request,response){
	...
	//connected to our database and saved the lists
	MongoClient.connect(url, function(err, db){
		if(err)throw err;
		
		var dbObj = db.db("lists");
		
		dbObj.collection("lists").save(request.body, function(err, result){
...
http.createServer(app).listen(3000, function(){
	console.log("List library server started on port 3000");
});